...
public class WorldModify : TerrariaPlugin
{
    ...
    private void WMCommand(CommandArgs args)
    {
        ...
        void ShowHelpText()
        {
            ...
            Utils.Pagination(args, ref lines, "/wm");
        }
        ...
        switch (args.Parameters[0].ToLowerInvariant())
        {
            ...
            case "find":
            case "f":
                WMFindTool.Manage(args);
                break;

            // 清理
            case "clear":
            case "c":
                WMClearTool.Manage(args);
                break;
            #endregion
            ...
            case "gps":
                ...
                {
                    op.SendErrorMessage("X坐标输入错误！");
                    return;
                }
                if (!int.TryParse(args.Parameters[2], out int y))
                {
                    op.SendErrorMessage("Y坐标输入错误！");
                    return;
                }
                op.SendInfoMessage($"坐标{{{x},{y}}} ≈ " + Utils.GetLocationDesc(x, y));
                break;
            ...
            case "xml":
                //ResHelper.DumpXML();
                Chest ch = Main.chest[0];
                Rectangle rect = new(ch.x - 59, ch.y - 38, 120, 68);
                if (rect.Contains(Main.spawnTileX, Main.spawnTileY))
                {
                    Console.WriteLine("在基地附近");
                }
                ...
                for (int rx = rect.X; rx < rect.Right; rx++)
                {
                    for (int ry = rect.Y; ry < rect.Bottom; ry++)
                    {
                        ...
                        if (tile.type == TileID.TeleportationPylon || tile.type == TileID.BewitchingTable)
                        {
                            Console.WriteLine("附近有晶塔 或 施法桌");
                            break;
                        }
                    }
                }
    ...
    public string GetWorldStatusDialog()
    {
        ...
        if (tGood > 0 && tEvil > 0 && tBlood > 0) return $"{tGood}%神圣 {tEvil}%腐化 {tBlood}%猩红";
        else if (tGood > 0 && tEvil > 0) return $"{tGood}%神圣 {tEvil}%腐化";
        else if (tGood > 0 && tBlood > 0) return $"{tGood}%神圣 {tBlood}%猩红";
        else if (tEvil > 0 && tBlood > 0) return $"{tEvil}%腐化 {tBlood}%猩红";
        else if (tEvil > 0) return $"{tEvil}%腐化";
        else if (tBlood > 0) return $"{tBlood}%猩红";
        else return "";
    }
    #endregion
